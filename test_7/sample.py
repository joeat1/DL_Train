    ...
    for i in range(1, len(layers)):
        if hasattr(layers[i], 'activation'):
            if layers[i].activation == replaced_layer:
                layers[i].activation = activation_fun()
        x = layers[i].output
...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    #conv_layer= [l for l in input_model.layers if l.name is layer_name][0]
    grad_model = tf.keras.Model(inputs=input_model.inputs , outputs=[input_model.get_layer(layer_name).output , input_model.output])
    with tf.GradientTape() as tape:
    ...
    #print(weights.shape, output.shape) # (512,) (14, 14, 512)
