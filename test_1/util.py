...
def draw_image_labels(img_file,anno_file,path_save,path_read):
    ...
    with open(os.path.join(path_read,anno_file)) as f_anno:
        ...
        for line in lines:
            ...
            cv2.circle(image, (int(x), int(y)),radius=1,color=(0,0,255),thickness=5)
            cv2.putText(image, str(name_id), (int(x)+10, int(y)+10), cv2.FONT_HERSHEY_SIMPLEX,0.4,(255,255,255),1)   
    cv2.imwrite(os.path.join(path_save,img_file),image)

#-------------对数据集文件夹root_data下文件夹进行遍历，对其中样本利用draw_image_labels绘制其关键点，进行标注，并将所有结果保存在root_save文件夹下-----
def draw_points(root_data,root_save):
    ...
    for rr,dn,files in os.walk(root_data):
        ...
        elif rr.split(os.path.sep)[-1] in flight_name and len(flight_name)!=0 and len(files)!=0:
            data_name=rr.split(os.path.sep)[-1]
            for img_file in files:
                if osp.splitext(img_file)[1]=='.jpg':
                    anno_file=img_file.replace(img_file[-4:],'.txt')
                    draw_image_labels(img_file,anno_file,os.path.join(root_save,data_name),os.path.join(root_data,data_name))
